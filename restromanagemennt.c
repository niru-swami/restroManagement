...
struct node *head_s;
// for customer to order food, view bill , cancel a food item etc. 
void customer_menu()
{
    printf("\n\t\t\t\t\t\t\t1. Place your order\n");
...
//for new customer
struct node* createcustomer(struct node *head,int data,int quantiity)
{
    newnode = (struct node*)malloc(sizeof(struct node));
    ...
    {
        newnode->data = data;
        newnode->Price = quantiity*(temp1->Price);
        newnode->quantiity = quantiity;
...
void displayList(struct node *head)
{
    ...
    else
    {
        ...
        while(temp1!=NULL)
        {
            if(temp1->quantiity==0)
                printf("\t\t\t\t\t\t\t%d\t%s\t%0.2f\n",temp1->data,temp1->NameOfFood,temp1->Price);
            else
            {
                printf("\t\t\t\t\t\t\t%d\t%s\t%d\t%0.2f\n",temp1->data,temp1->NameOfFood,temp1->quantiity,temp1->Price);
            }
...
// view total sales
struct node* totalsales(int data,int quantiity)
{
    newnode = (struct node*)malloc(sizeof(struct node));
    int flag = 0;
    ...
    else
    {
        ...
        if(flag==1)
        {
            temp->quantiity += newnode->quantiity;
...
void calculatetotsales()
{
    ...
    while(temp!=NULL)
    {
        head_s = totalsales(temp->data, temp->quantiity);
        temp=temp->next;
    }
}
// for deleting a user
...
// to cancel a item from ordered list
int deleteacustomer()
{
    printf("\n\t\t\t\t\tEnter serial no. of the food item which is to be deletead: ");
    int num;
    ...
    while(1)
    {
        ...
        switch (opt)
        {
            case 1:
                ...
                int n;
                scanf("%d",&n);
                printf("\t\t\t\t\t\tEnter quantiity: ");
                int quantiity;
                scanf("%d",&quantiity);
                headc = createcustomer(headc, n, quantiity);
                break;
            case 2:
